...
class Ajaxify {
	constructor(options) {
		...
		Ay.parse = (s, pl) => (
			...
			pl.insertAdjacentHTML('afterbegin', s),
			pl.firstElementChild
		); // HTML parser
		Ay.trigger = (t, e) => {
		...
		function _copyAttributes(el, $S, flush) {
			//copy all attributes of element generically
			if (flush)
				[...el.attributes].forEach((e) => el.removeAttribute(e.name)); //delete all old attributes
			[...$S.attributes].forEach(
				(e) =>
					e.nodeValue == 'ajy-body' ||
					el.setAttribute(e.nodeName, e.nodeValue)
			); //low-level insertion
		}
			...
			constructor(h) {
				...
				_.list =
					typeof h === 'string' && h.length > 0
						? h.split(', ')
						: false; //hints are passed as a comma separated string
				_.find = (t) =>
					!t || !_.list ? false : _.list.some((h) => t.iO(h)); //iterate through hints within passed text (t)
			}
		}
		...
		class GetPage {
			constructor() {
				...
				let _lSel = ($t) => (
						Ay.pass++,
						_lEls($t),
						qa('body > script').forEach((e) =>
							e.classList.contains(inlineclass)
					...
					_lEls = ($t) =>
						...
						$t.forEach(function ($el) {
							_ld(
								$el,
								qs('#' + $el.getAttribute('id'), Ay.cache.g())
							);
						}),
					_isBody = ($t) =>
						$t[0].tagName.toLowerCase() == 'body' &&
						(_ld(bdy, qs('#ajy-body', Ay.cache.g())), 1),
					_lAjax = (hin, pre) => {
						var ispost = Ay.Rq('is');
						if (pre) rt = 'p';
						else rt = 'c';
					...
					_cache = (href, h, err) =>
						Ay.cache.s(Ay.parse(_parseHTML(h))) &&
						(Ay.pages.p([href, Ay.cache.g()]), 1) &&
						cb &&
						cb(err),
					_isHtml = (x) =>
						(ct = x.headers.get('content-type')) &&
						(ct.iO('html') || ct.iO('form-')),
					_parseHTML = (h) =>
		...
		class Scripts {
			constructor() {
				...
				this.a = function (o) {
					...
					if (o === 'c')
						return Ay.s.canonical && $s.can
							? $s.can.getAttribute('href')
							: false;
				...
				let _allstyle = ($s) =>
					...
					_onetxt = ($s) =>
						!(txt = $s.textContent).iO(').ajaxify(') &&
						!txt.iO('new Ajaxify(') &&
						((Ay.s.inline && !Ay.h.inlineskip.find(txt)) ||
							$s.classList.contains('ajaxy') ||
					...
					_addtxt = ($s) => {
						...
						if (
							Ay.s.inlineappend ||
							($s.getAttribute('type') &&
								!$s.getAttribute('type').iO('text/javascript'))
						)
							try {
								return _apptxt($s);
							} catch (e) {}
					...
					_addstyle = (t) =>
						qs('head').appendChild(
							Ay.parse('<style>' + t + '</style>')
						),
					_addScripts = ($s) => (
						Ay.addAll($s.c, 'href'), Ay.addAll($s.j, 'src')
					);
			}
		}
			...
			constructor() {
				...
				this.a = function ($this, pk) {
					...
					if (!Ay.pass)
						_newArray(
							$this
						); //Fill new array on initial load, nothing more
					else
						$this.forEach(function (s) {
							...
							if (
								PK != 'href' &&
								!$t.classList.contains('no-ajaxy')
							)
								Ay.scripts($t); //Inline JS script? -> inject into DOM
						});
				};
				let _allScripts = ($t) => $t.forEach((e) => _iScript(e)),
					_newArray = ($t) =>
						$t.forEach((e) =>
							(url = e.getAttribute(PK)) ? $scriptsO.push(url) : 0
						),
					_classAlways = ($t) =>
						$t.getAttribute('data-class') == 'always' ||
						Ay.h.alwayshints.find(url),
					_iScript = ($S) => {
						url = $S.getAttribute(PK);

						if (PK == 'href')
							return qs('head').appendChild(
								Ay.parse(linki.replace('*', url))
							);
					...
					_removeScript = () =>
						qa(
							(PK == 'href' ? linkr : scrr).replace('!', url)
		...
		class RQ {
			constructor() {
				...
				this.a = function (o, p, t) {
					...
					if (o === 'd') {
						//set / get internal "d" (data for central $.ajax())
						if (p) data = p;
						return data;
					}
					...
					if (o === 'c')
						return can && can !== p && !p.iO('#') && !p.iO('?')
							? can
							: p; //get internal "can" ("href" of canonical URL)
				};
				let _setE = (p, t) =>
					(h =
						typeof (e = p) !== 'string'
		...
		class Pronto {
			constructor() {
				...
				let _init_p = () => {
					...
					_preftime = (t, e) => (
						_prefstop(),
						(ptim = setTimeout(() => _prefetch(t, e), pd))
					...
					_doRender = () => {
						Ay.trigger('load');
						if (Ay.s.bodyClasses)
							_copyAttributes(bdy, Ay.fn('body'), true);
						...
						if ((title = Ay.fn('title')))
							qs('title').innerHTML = title.innerHTML;
						Ay.Rq('C', Ay.fn('-', $gthis));
						Ay.frms('a');
		...
		let run = () => {
				if (Ay.running) return;
				else Ay.running = true;
...
class DetScripts {
	...
	x(v) {
		return Array.prototype.filter.call(this.lk, (e) =>
			e.getAttribute('rel').iO(v)
		);
...
class Scrolly {
	...
	l() {
		...
		if (Ay.s.scrolltop == 's')
			this.s(Ay.offsets.l(o)); //smart scroll -> lookup and restore offset
		else Ay.s.scrolltop && this.s(0); //scrolltop true -> scroll to top of page
	}
	s(o) {
		setTimeout(() => window.scrollTo(0, o), Ay.s.scrollDelay);
	} //scroll to offset
}
...
class HApi {
	...
	p(h) {
		let c = this.u(h);
		if (c !== window.location.href)
			history.pushState({ url: c }, 'state-' + c, c);
	} //perform pushState
	u(h) {
